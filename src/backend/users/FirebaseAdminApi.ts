...
if(
  fs.existsSync(path.join(process.cwd(), 'config/serviceAccount.json')) &&
  fs.existsSync(path.join(process.cwd(), 'config/firebaseConfig.json'))
) {
...
// Define paths to the service account and Firebase config files
  ...
  config = JSON.parse(fs.readFileSync(filePathConfig, 'utf-8'));
